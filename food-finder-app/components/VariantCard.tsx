...
const VariantCard: React.FC<Props> = ({ dishes, isFavorite, onToggleFavorite, index }) => {
  const total = dishes.reduce((sum, d) => sum + d.price, 0);
  const isGreen = index !== undefined && index % 2 === 1;
  return (
    <View style={[styles.card, isGreen ? styles.cardGreen : styles.cardOrange]}>
      ...
      <Text style={styles.variantTitle}>{index !== undefined ? `${index + 1} вариант` : 'Вариант'}</Text>
      {dishes.map(dish => (
