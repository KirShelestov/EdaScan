...
const InputForm = () => {
  ...
  const [selectedDishes, setSelectedDishes] = useState<number[]>([ANY_DISH_ID]);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.label}>Введите сумму, на которую хотите поесть</Text>
      <TextInput
        style={styles.input}
      ...
      <View style={styles.gridContainer}>
        {grid.map((row, rowIdx) => (
          <View key={rowIdx} style={styles.gridRow}>
            {row.map((cell, colIdx) => {
              if (cell === null) {
                return <View key={colIdx} style={[styles.chip, { backgroundColor: 'transparent', borderWidth: 0, elevation: 0 }]} />;
              }
              if (cell === 'expand') {
                return (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.8}
                  >
                    <Text style={styles.expandButtonText}>Развернуть{"\n"}полный список блюд</Text>
                  </TouchableOpacity>
                );
              }
              ...
              return (
                <TouchableOpacity
                  key={dishId}
                  style={[styles.chip, selectedDishes.includes(dishId) ? styles.chipSelected : styles.chipOutline]}
                  onPress={() => handleToggleDish(dishId)}
                >
                  <Text style={selectedDishes.includes(dishId) ? styles.chipTextSelected : styles.chipText}>{dishMap[dishId]}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
  ...
  activeOpacity={0.8}
>
  <Text style={styles.submitButtonText}>Подобрать прием пищи</Text>
</TouchableOpacity>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  style={[
                    styles.chip,
                    selectedDishes.includes(item.id) ? styles.chipSelected : styles.chipOutline,
                    styles.modalChip, // только для модалки
                  ]}
                  onPress={() => handleToggleDish(item.id)}
                >
                  <Text
                    style={selectedDishes.includes(item.id) ? styles.chipTextSelected : styles.chipText}
                    numberOfLines={1}
                  >
                    {item.name}
                  </Text>
